...
::: {.notes}
- I'm part of our annual giving mass solicitations team, primarily supporting our direct mail program
- I'm a 2-time VU alum: class of 2016 with a BA in English literature and recently class of 2026 MS in Business Analytics
...
- Continuity: can be case-by-case---if there is an important categorical order that must be maintained (e.g., campaign pillars), we may forgo this principle
:::
...
High-level walkthrough using donor dataset for 2 types of charts: bar and line

Demonstrate Schwabish's 5 guidelines to generate two simple, polished visuals
...
regional_donors <- donors %>% 
  ...
  filter(last_gift != 0 & geographic_region != 'International' & !is.na(designation) & year == max(year, na.rm = TRUE)) %>% 
  group_by(geographic_region, primary_constituent_type, designation) %>% 
  summarize(dollars_raised = sum(gift_amount))

ggplot(regional_donors, aes(x = reorder(designation, -dollars_raised), y = dollars_raised)) +
  geom_col()
...
Placeholder slide for resources  -->
